...
abstract contract EIP712MetaTransaction is ContextUpgradeable {
    ...
    function executeMetaTransaction(address userAddress,
        ...
        require(destinationFunctionSig != msg.sig, "Mt1"); //functionSignature can not be of executeMetaTransaction method
        MetaTransaction memory metaTx = MetaTransaction({
        nonce : nonces[userAddress],
        from : userAddress,
        functionSignature : functionSignature
        });
        require(verify(userAddress, metaTx, sigR, sigS, sigV), "Mt2");//Signer and signature do not match
        nonces[userAddress] = nonces[userAddress].add(1);
        // Append userAddress at the end to extract it from calling context
        (bool success, bytes memory returnData) = address(this).call(abi.encodePacked(functionSignature, userAddress));

        require(success, "Mt3");//Function call not successful
    ...
    function verify(address user, MetaTransaction memory metaTx, bytes32 sigR, bytes32 sigS, uint8 sigV) internal view returns (bool) {
        ...
        require(signer != address(0), "Mt4");//bad signature
        return signer == user;
    }
