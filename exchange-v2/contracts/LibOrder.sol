...
library LibOrder {
    ...
    function validate(LibOrder.Order memory order) internal view {
        require(order.start == 0 || order.start < block.timestamp, "e13");//Order start validation failed
        require(order.end == 0 || order.end > block.timestamp, "e14");//Order end validation failed
    }
}
